...
//        "password": "Alcatel.01",   // The Rainbow associated password to use
        ...
        "login": "vincent04@vbe.test.openrainbow.net",  // The Rainbow email account to use
...
class EngineVincent01 {

    constructor() {
        // Instantiate the SDK
        this.rainbowSDK = new RainbowSDK(optionsVincent01);
    ...
    init () {
        ...
        that.rainbowSDK.events.on('rainbow_onready', (data) => {
            ...
            that.logger.log("debug", "EngineVincent01 - rainbow_onready - rainbow onready", data, that.userinfo);
        });

        that.rainbowSDK.events.on('rainbow_onstarted', (data) => {
            // do something when the SDK has been started
            that.logger.log("debug", "EngineVincent01 - rainbow_onstarted - rainbow onstarted", data);
        });


        that.rainbowSDK.events.on('rainbow_oncallupdated', (data) => {
            that.logger.log("debug", "EngineVincent01 - rainbow_oncallupdated - rainbow call updated. State : " + that.logger.colors.cyan(data.status.value) + ", displayName : " + data.contact.displayName, data);
            that.saveCall(data);

            if (data.status === Call.Status.RINGING_INCOMMING) {
                that.rainbowSDK.telephony.answerCall(data).then((dataanswerCall) => {
                    that.logger.log("debug", "EngineVincent01 - [answerCall] ", dataanswerCall);
                }).catch((error) => {
                    that.logger.log("debug", "EngineVincent01 - [answerCall] error ", error);
                });
            }
        ...
        that.rainbowSDK.events.on('rainbow_onvoicemessageupdated', (data) => {
            that.logger.log("debug", "EngineVincent01 - rainbow_onvoicemessageupdated - rainbow voice message updated.", data);
        });

        that.rainbowSDK.events.on('rainbow_onbubbleinvitationreceived', (bubble) => {
            that.logger.log("debug", "EngineVincent01 - rainbow_onbubbleinvitationreceived - rainbow event received.", bubble, that.userinfo.loggedInUser);
            ...
            /* this.rainbowSDK.bubbles.acceptInvitationToJoinBubble(bubble).then((updatedBubble) => {
                that.logger.log("debug", "EngineVincent01 - acceptInvitationToJoinBubble - sent." ), bubble);
                // Do something once the invitation has been accepted
            }).catch((err) => {
                // Do something in case of error
                that.logger.log("error", "EngineVincent01 - acceptInvitationToJoinBubble - error : " ), err);
            });
            // */
        });

        that.rainbowSDK.events.on('rainbow_onmessagereceiptreceived', (data) => {
            that.logger.log("debug", "EngineVincent01 - rainbow_onmessagereceiptreceived - rainbow event received.", data);
        });

        that.rainbowSDK.events.on('rainbow_onmessagereceived', (data) => {
            that.logger.log("debug", "EngineVincent01 - rainbow_onmessagereceived - rainbow event received.", data);
        });

        that.rainbowSDK.events.on('rainbow_onuserinvitereceived', (data) => {
            that.logger.log("debug", "EngineVincent01 - rainbow_onuserinvitereceived - rainbow event received.", data);
        });

        that.rainbowSDK.events.on('rainbow_onbubblecustomdatachanged', (data) => {
            that.logger.log("debug", "EngineVincent01 - rainbow_onbubblecustomdatachanged - rainbow event received.", data);
            that.getBubbleByJid();
     ...
     start () {
        ...
        return that.rainbowSDK.start().then((result) => {
            ...
            that.logger.log("debug", "EngineVincent01 - rainbow SDK started result : " + JSON.stringify(result)); //this.logger.colors.green(JSON.stringify(result)));
...
// */
         ...
         /*    then((msg) => {
            ...
            /* .then((msg) => {
                    ...
                    that.logger.log("debug", "EngineVincent01 - sendMessageToJid.then() message sent", msg);
                }).catch((err) => {
...
module.exports = EngineVincent01;